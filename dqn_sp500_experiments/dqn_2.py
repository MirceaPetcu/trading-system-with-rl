    ...
    def play_episode(self, env, agent, no_episode, debug=False):
        ...
        while True:
            ...
            if done:
                ...
                return score, total_profit, info
...
class Agent():
    ...
    def get_action(self, state):
        ...
        state_tensor = torch.tensor(np.array(state).reshape(1, -1, 10), dtype=torch.float32)
    ...
    def fit_model(self, states, labels):
        ...
        """
        self.model.train()
        tensor_dataset = TensorDataset(torch.tensor(np.array(states), dtype=torch.float32),
